...
import com.terminal.SubstringFinder.FindResult;
...
/**
 * JediTerm com.terminal widget with UI implemented in Swing.
 * <p/>
 */
public class JediTermWidget extends JPanel implements TerminalSession, TerminalWidget, TerminalActionProvider {
  ...
  public JediTermWidget(@NotNull SettingsProvider settingsProvider) {
    this(80, 24, settingsProvider, Lists.newArrayList());
  }

  public JediTermWidget(@NotNull SettingsProvider settingsProvider, List<JediEmulator.NonControlCharListener> nonControlCharListeners) {
    this(80, 24, settingsProvider, nonControlCharListeners);
  }

  public JediTermWidget(Dimension dimension, SettingsProvider settingsProvider) {
    this(dimension.width, dimension.height, settingsProvider, Lists.newArrayList());
  }

  public JediTermWidget(int columns, int lines, SettingsProvider settingsProvider, List<JediEmulator.NonControlCharListener> nonControlCharListeners) {
  ...
  protected TerminalStarter createTerminalStarter(JediTerminal terminal, TtyConnector connector) {
    return new TerminalStarter(terminal, connector, new TtyBasedArrayDataStream(connector), nonControlCharListeners);
  }
  ...
  protected interface SearchComponent {
    ...
    void nextFindResultItem(FindResult.FindItem selectedItem);

    void prevFindResultItem(FindResult.FindItem selectedItem);
  }
  ...
  class EmulatorTask implements Runnable {
    public void run() {
      try {
        ...
        LOG.error("Exception running com.terminal", e);
      } finally {
        try {
  ...
  public class SearchPanel extends JPanel implements SearchComponent {
    ...
    @Override
    public void nextFindResultItem(FindResult.FindItem selectedItem) {
      updateLabel(selectedItem);
    }

    @Override
    public void prevFindResultItem(FindResult.FindItem selectedItem) {
      updateLabel(selectedItem);
    }

    private void updateLabel(FindResult.FindItem selectedItem) {
  ...
  public void addNonControlCharacterListener(JediEmulator.NonControlCharListener nonControlCharListener) {
    //this.myTerminal.add
  }
}
